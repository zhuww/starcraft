...
#default = set([SCV, Marine, Supply_Depot, Refinery, Barracks, Barracks_Tech_Lab, Orbital_Command, MULE, Extra_Supplies, Marauder, Factory, Factory_Reactor, Factory_Tech_Lab, Swap_Barracks_Factory_Tech_Lab, Swap_Barracks_Factory_Reactor, Hellion, Infernal_Pre_igniter])
default = set([SCV, Marine, Supply_Depot, Refinery, Barracks, Barracks_Tech_Lab, Orbital_Command, MULE, Extra_Supplies, Factory, Factory_Reactor, Factory_Tech_Lab, Swap_Barracks_Factory_Tech_Lab, Swap_Barracks_Factory_Reactor, Siege_Tank, Siege_Tech])
...
def Fitness(simulator):
    ...
    for log in simulator.log:
        if log[0] == Marine:m.append(float(log[2])/simulator.timeout)
        if log[0] == Hellion:h.append(float(log[2])/simulator.timeout)
        ...
        if log[0] == Siege_Tech:i.append(float(log[2])/simulator.timeout)
        if log[0] == Siege_Tank:t.append(float(log[2])/simulator.timeout)
    ...
    if len(m) == 0: q = 0
    else:q =  1 - sum(m)/len(m)
    if len(t) == 0: x = 0
    else:x =  1 - sum(t)/len(t)
    if len(h) == 0: y = 0
    else:y =  1 - sum(h)/len(h)
    if len(s) == 0: z = 0
    ...
    #return len(m)*5 + 0.1*len(s) + 10*len(i) + len(f) +  y + z + v + u
    return (4*len(t)+x)*(1.+1.*ipi) * (len(m)+q)  
...
def TheMaker(simulator, geno, allowed, KeyUnit=None):
    ...
    P = geno([Item(o, simulator) for o in [SCV, SCV, SCV, SCV, Supply_Depot, Refinery, Barracks, SCV, SCV, SCV]], simulator=simulator)
    while True:
        #try:
        if len(allowed) == 0: 
            break
